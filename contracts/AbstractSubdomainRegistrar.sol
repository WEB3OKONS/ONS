...
contract AbstractSubdomainRegistrar is RegistrarInterface {
    ...
    constructor(ONS _ons) public {
        ons = _ons;
        registrar = ons.owner(TLD_NODE);
        registrarOwner = msg.sender;
    }

    function doRegistration(bytes32 node, bytes32 label, address subdomainOwner, Resolver resolver) internal {
        // Get the subdomain so we can configure it
        ons.setSubnodeOwner(node, label, address(this));

        bytes32 subnode = keccak256(abi.encodePacked(node, label));
        // Set the subdomain's resolver
        ons.setResolver(subnode, address(resolver));

        // Set the address record on the resolver
        resolver.setAddr(subnode, subdomainOwner);

        // Pass ownership of the new subdomain to the registrant
        ons.setOwner(subnode, subdomainOwner);
    }
    ...
    /**
     * @dev Sets the resolver record for a name in ONS.
