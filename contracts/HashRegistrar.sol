...
contract HashRegistrar is Registrar {
    ...
    modifier registryOpen() {
        require(now >= registryStarted && now <= registryStarted + (365 * 4) * 1 days && ons.owner(rootNode) == address(this));
        _;
    }
     ...
     *
     * @param _ons The address of the ONS
     * @param _rootNode The hash of the rootnode.
     */
    constructor(ONS _ons, bytes32 _rootNode, uint _startDate) public {
        ons = _ons;
        rootNode = _rootNode;
    ...
    function releaseDeed(bytes32 _hash) external onlyOwner(_hash) {
        ...
        require(now >= h.registrationDate + 365 days || ons.owner(rootNode) != address(this));
    ...
    function transferRegistrars(bytes32 _hash) external onlyOwner(_hash) {
        address registrar = ons.owner(rootNode);
     ...
     * @param salt A random value to onsure secrecy of the bid
    ...
    function _tryEraseSingleNode(bytes32 label) internal {
        if (ons.owner(rootNode) == address(this)) {
            ons.setSubnodeOwner(rootNode, label, address(this));
            bytes32 node = keccak256(abi.encodePacked(rootNode, label));
            ons.setResolver(node, address(0x0));
            ons.setOwner(node, address(0x0));
        }
    }
    ...
    function _eraseNodeHierarchy(uint idx, bytes32[] memory labels, bytes32 node) internal {
        ...
        ons.setSubnodeOwner(node, labels[idx], address(this));
    ...
    /**
     * @dev Assign the owner in ONS, if we're still the registrar
     *
    ...
    function trySetSubnodeOwner(bytes32 _hash, address _newOwner) internal {
        if (ons.owner(rootNode) == address(this))
            ons.setSubnodeOwner(rootNode, _hash, _newOwner);
    }
